...
const deploymentPaginationSchema = createPaginationInputSchema(['createdAt'] as const, deploymentFilterSchema);

export const deploymentRouter = (router: Router, procedure: Procedure) => {
	return router({
		list: procedure.input(deploymentPaginationSchema).query(async ({ ctx, input }) => {
